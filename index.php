...
class Controller {
    ...
    /**
     * For the given session, sets the session cookie value. 
     * If the session is null, forces cookie expiry.
     *
     * @param object    a session object (see orm.php)
     */
    ...
    /**
     * Attempts to load the session object for the set session cookie.
     * 
     * @return object   a session object (see orm.php), NULL if no session or failed to load.
     */
    public function get_session() {
        if($this->current_session !== NULL) {
            return $this->current_session;
        }
        if(!array_key_exists('opmb_session', $_COOKIE)) {
            return NULL;
        }
        ...
        $session = session::select_by_code($key);
        if($session === NULL || $session === FALSE) {
            return NULL;
        }
        $test = md5(strtolower($session->code . $session->id . $session->created));
    ...
    protected function get_age_string ($string) {
        ...
        if ($time < 120) return '1 minute ago';
        if ($time < 3600) return intval($time / 60).' minutes ago';
        if ($time < 7200) return '1 hour ago';
        if ($time < 86400) return intval($time / 3600).' hours ago';
        if ($time < 172800) return '1 day ago';
        if ($time < 2592000) return intval($time / 86400).' days ago';
        if ($time < 5184000) return '1 month ago';
        if ($time < 31536000) return intval($time / 2592000).' months ago';
        if ($time < 63072000) return '1 year ago';
        return intval($time / 31536000).' years ago';
    }
}
